...
classdef OperatorXLineCommand
    %OperatorXLineCommand Conversion of CR_operatorXLine.m
    ...
    methods
        %examine variable
        function [outputStr, errorFlag] = operatorXLineCommand(obj, varargin)
          if ~isequal(nargin,3)
            ...
            if ~exist('showDialogs','var') || showDialogs==1
                modalDlg{end+1} = variableTwo;
                try %#ok<TRYNC>
                   if handles.speak
                       Speak(handles.speaker,variableTwo);
                       outputStr = sprintf('OperatorXLine Command Success.');
                   end
                end
            end
          end
